...
def run_sanity_check(test_dir):
    # assert path.isdir(test_dir), FAIL_COLOR+f"No direcotry named {test_dir} found in
    # {os.getcwd()}"
    print(
        """
        This script will perform a sanity test to ensure your code meets the criteria"
        in the rubric.\n"""
    )
    print(
        """Please enter the path to the file that contains your test cases for the GET()
        and POST() methods"""
    )
    ...
    if not test_functions_for_get:
        ...
        print(
            ...
            This MUST test both the status code as well as the contents of the request
            object.\n"""
        )
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_CODE:
            ...
            print(
                FAIL_COLOR
                + """Your test case for GET() does not seem to be testing the response
                code.\n"""
            )
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            ...
            print(
                FAIL_COLOR
                + """Your test case for GET() does not seem to be testing the CONTENTS
                of the response.\n"""
            )
    ...
    if not test_functions_for_post:
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_CODE:
            ...
            print(
                FAIL_COLOR
                + """One or more of your test cases for POST() do not seem to be testing
                the response code.\n"""
            )
    ...
    print(
        WARN_COLOR
        + """You should still check your work against the rubric to ensure you meet the
        criteria."""
    )
